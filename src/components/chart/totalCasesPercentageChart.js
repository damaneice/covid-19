...
const createCasesChartData = (county, stateCasesGroupedByDate) => {
  ...
  for (let i = 0; i < county.chart.length; i++) {
    ...
    const statesCases = stateCasesGroupedByDate[countyCasesDate].totalCases
    chartData.push({
      x: i,
      y: (county.chart[i].totalCases / statesCases) * 100 || 0,
...
const TotalCasesPercentageChart = ({
  counties,
  selectedNames,
  updatedDate,
}) => {
  ...
  return (
    <div>
      {selectedCounties.length > 0 && (
        <ChartWithLegend
          name="Percentage of State's Total Cases"
...
export default TotalCasesPercentageChart
